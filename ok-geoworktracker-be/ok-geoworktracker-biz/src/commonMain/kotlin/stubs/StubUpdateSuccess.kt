...
fun ICorChainDsl<GwtrContext>.stubUpdateSuccess(title: String, corSettings: GwtrCorSettings) = worker {
    this.title = title
    this.description = """
    ...
    handle {
        logger.doWithLogging(id = this.requestId.asString(), LogLevel.DEBUG) {
            ...
            val stub = GwtrTicketStub.prepareResult {
                ticketRequest.id.takeIf { it != GwtrTicketId.NONE }?.also { this.id = it }
