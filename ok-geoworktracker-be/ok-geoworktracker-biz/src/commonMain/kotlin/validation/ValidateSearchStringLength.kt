...
fun ICorChainDsl<GwtrContext>.validateSearchStringLength(title: String) = chain {
    this.title = title
    this.description = """
    ...
    worker {
        ...
        on { state == GwtrState.RUNNING && ticketFilterValidating.searchString.length in (1..2) }
        handle {
            fail(
                errorValidation(
                    field = "searchString",
    ...
    worker {
        ...
        this.description = this.title
        on { state == GwtrState.RUNNING && ticketFilterValidating.searchString.length > 100 }
        handle {
            fail(
                errorValidation(
                    field = "searchString",
